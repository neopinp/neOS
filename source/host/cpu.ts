  ...
  export class Cpu {
    ...
    public cycle(): void {
      ...
      if (typeof base === "undefined" || typeof limit === "undefined") {
        console.error(
          `Invalid process boundaries: base=${base}, limit=${limit}`
        );
        this.isExecuting = false;
        return;
      }
      ...
      console.log(
        `Fetching instruction at PC: ${this.PC}, Base: ${base}, Limit: ${limit}`
      );
      ...
      try {
        ...
        if (physicalAddress < base || physicalAddress > limit) {
          ...
          throw new Error(
            `Memory access violation at address ${physicalAddress}.`
          );
        }
        ...
        if (typeof instruction === "undefined") {
          console.error(
            `Fetched instruction is undefined at address ${physicalAddress}`
          );
          throw new Error(
            `Undefined instruction at address ${physicalAddress}`
          );
        }
    ...
    public execute(instruction: number): void {
      ...
      switch (instruction) {
        case 0xa9: // LDA: Load Accumulator with a constant
          console.log(
            `LDA: Loading value from address ${
        ...
        case 0x8d: // STA: Store Accumulator in memory
          ...
          console.log(
            `STA: Storing Acc value ${this.Acc} at address ${
              address + neOS.CurrentProcess.base
            }`
          );
          this.memoryAccessor.write(
            ...
            neOS.CurrentProcess.base,
            neOS.CurrentProcess.limit
          ); // Write the Accumulator value to memory
          this.PC += 3;
        ...
        case 0xa2: // LDX: Load X register with a constant
          console.log(
            `LDX: Loading value from address ${
              neOS.CurrentProcess + this.PC + 1
            }`
          );
          value = this.memoryAccessor.read(
            neOS.CurrentProcess.base + this.PC + 1,
